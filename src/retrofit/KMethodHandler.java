...
final class KMethodHandler<T> {
	...
	static KMethodHandler<?> create(Method method, OkHttpClient client, BaseUrl baseUrl, List<CallAdapter.Factory> callAdapterFactories, List<Converter.Factory> converterFactories) {
		CallAdapter<Object> callAdapter = (CallAdapter<Object>) createCallAdapter(method, callAdapterFactories);
		Converter<Object> responseConverter = (Converter<Object>) createResponseConverter(method, callAdapter.responseType(), converterFactories);
	...
	private static CallAdapter<?> createCallAdapter(Method method, List<CallAdapter.Factory> adapterFactories) {
	...
	private static Converter<?> createResponseConverter(Method method, Type responseType, List<Converter.Factory> converterFactories) {
	...
	private KMethodHandler(OkHttpClient client, RequestFactory requestFactory, CallAdapter<T> callAdapter, Converter<T> responseConverter) {
		this.client = client;
