...
import com.kubeiwu.httphelper.retrofit.KGsonConverter;
 ...
 * <ul>
 * <li>{@link retrofit.http.FormUrlEncoded @FormUrlEncoded} - Form-encoded data with key-value pairs specified by the {@link retrofit.http.Field @Field} parameter annotation.
 * <li>{@link retrofit.http.Multipart @Multipart} - RFC 2387-compliant multi-part data with parts specified by the {@link retrofit.http.Part @Part} parameter annotation.
 * </ul>
 * <p>
...
//	}
	...
	private KRestAdapter(Endpoint server, Client.Provider clientProvider, Executor httpExecutor, Executor callbackExecutor, RequestInterceptor requestInterceptor, Converter converter, Profiler profiler, ErrorHandler errorHandler, Log log, LogLevel logLevel) {
		this.server = server;
	...
	public static class Builder {
		 ...
		 *            Executor on which any {@link Callback} methods will be invoked. If this argument is {@code null} then callback methods will be run on the same thread as the HTTP client.
		 */
		...
		/** Create the {@link KRestAdapter} instances. */
		public KRestAdapter build() {
			if (endpoint == null) {
				throw new IllegalArgumentException("Endpoint may not be null.");
			}
			ensureSaneDefaults();
			return new KRestAdapter(endpoint, clientProvider, httpExecutor, callbackExecutor, requestInterceptor, converter, profiler, errorHandler, log, logLevel);
		}
